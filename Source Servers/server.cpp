        ...
        if (!(ptrClient->isLeasing)) {
            ...
            ptrClient->rozetka->async_read_some(buffer(ptrClient->requestbuffer),[ptrClient](const boost::system::error_code& ec, size_t bytes_transferred){
                ...
                if (ec) {
                    ...
                    reqBuf[static_cast<int>(bytes_transferred)] = '\n';
                    
                    if (strcmp(reqBuf, "setInUnAuth") == 1) { // этот запрос выполнится при своравчивании приложения либо же при потери связи между устр.
